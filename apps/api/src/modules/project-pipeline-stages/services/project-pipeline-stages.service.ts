...
export class ProjectPipelineStagesService {
  ...
  async create(projectId: string, value: string, sortOrder: number) {
    const row = await this.repository.createProjectPipelineStage(projectId, value, sortOrder);
    if (!row) {
  ...
  async update(projectId: string, id: string, values: Partial<{ value: string; sortOrder: number }>) {
    const row = await this.repository.updateProjectPipelineStage(projectId, id, values);
    if (!row) {
      throw new NotFoundException('Pipeline stage not found, or not owned by this project');
    }
    return row;
  }
