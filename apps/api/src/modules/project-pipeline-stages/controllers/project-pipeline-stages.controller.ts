...
import {
  ...
  export class ProjectPipelineStagesController {
    ...
    private async assertOwner(tenantId: string, projectId: string, req: AuthenticatedRequest) {
      ...
      if (project.userId !== user.id) {
        throw new ForbiddenException('Only the project owner can manage its pipeline stages');
      }
    }
  
    @ApiOperation({ summary: 'List pipeline stages available to this project (base + custom)' })
    ...
    @ApiOperation({ summary: 'Create a custom pipeline stage for this project (owner only)' })
    ...
    @ApiOperation({ summary: 'Rename or reorder a custom pipeline stage (owner only)' })
    @UseGuards(JwtAuthGuard, TenantMemberGuard)
    @Patch(':id')
    async update(
      @Param('tenantId') tenantId: string,
