...
describe('MyProjectsController', () => {
  ...
  it('findOne() delegates to findOneForCaller with the project id and caller id', async () => {
    ...
    expect(projectsService.findOneForCaller).toHaveBeenCalledWith('project-1', 'user-1');
    expect(result).toEqual({ id: 'project-1' });
  });

  it('update() delegates to updateForCaller with the project id, caller id, and body', async () => {
    projectsService.updateForCaller.mockResolvedValue({ id: 'project-1', title: 'Updated' });
    const result = await controller.update('project-1', req(), { title: 'Updated' });
    expect(projectsService.updateForCaller).toHaveBeenCalledWith('project-1', 'user-1', {
      title: 'Updated',
    });
  ...
  it('archive() delegates to archiveForCaller with the project id and caller id', async () => {
    ...
    expect(projectsService.archiveForCaller).toHaveBeenCalledWith('project-1', 'user-1');
    expect(result).toEqual({ id: 'project-1' });
  });
});
