...
import {
  ...
  export class ModulePipelineStagesController {
    ...
    private async assertOwner(tenantId: string, moduleId: string, req: AuthenticatedRequest) {
      ...
      const membership = await this.moduleCollaboratorsRepository.findByModuleAndUser(
        tenantId,
        moduleId,
        user.id,
      );
      if (!membership) {
        throw new ForbiddenException('Only the module owner can manage its pipeline stages');
      }
    ...
    @ApiOperation({ summary: 'List pipeline stages available to this module (base + custom)' })
    ...
    @ApiOperation({ summary: 'Create a custom pipeline stage for this module (owner only)' })
