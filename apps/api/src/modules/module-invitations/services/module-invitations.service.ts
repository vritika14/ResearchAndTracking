...
export class ModuleInvitationsService {
  ...
  async list(moduleId: string) {
    ...
    async preview(rawToken: string) {
      ...
      const moduleTitle = (titleResult.rows[0] as { title: string | null } | undefined)?.title ?? null;
    ...
    async accept(rawToken: string, userId: string, userEmail: string) {
      ...
      const module = await this.modulesRepository.findByIdGlobal(invitation.moduleId);
      if (!module) {
