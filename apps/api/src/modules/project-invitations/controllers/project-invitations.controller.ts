  ...
  export class ProjectInvitationsController {
    ...
    private async assertOwner(tenantId: string, projectId: string, req: AuthenticatedRequest) {
      ...
      if (project.userId !== user.id) {
        throw new ForbiddenException('Only the project owner can manage invitations');
      }
      return user;
    }
  
    @ApiOperation({ summary: 'List pending invitations for this project (owner only)' })
    ...
    @ApiOperation({ summary: 'Invite someone to collaborate on this project (owner only)' })
