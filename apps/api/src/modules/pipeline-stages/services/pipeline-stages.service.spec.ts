...
describe('PipelineStagesService', () => {
  ...
  it('lists the tenant-scoped project pipeline stage pool', async () => {
    const stages = [{ id: 's1', value: 'Draft' }];
  ...
  it('creates a custom stage scoped to the tenant', async () => {
    ...
    await service.create('tenant-1', 'Peer Review', 3);
  ...
  it('throws when creation fails to return a row', async () => {
    ...
    await expect(service.create('tenant-1', 'Peer Review', 3)).rejects.toThrow(
      ConflictException,
    );
  });
