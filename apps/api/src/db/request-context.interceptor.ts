...
import {
  ...
  export class RequestContextInterceptor implements NestInterceptor {
    ...
    intercept(context: ExecutionContext, next: CallHandler): Observable<unknown> {
        console.log('DEBUG - intercept() called for:', context.switchToHttp().getRequest().url);
    ...
    private async handle(context: ExecutionContext, next: CallHandler) {
      ...
      console.log('DEBUG - interceptor req.user:', req.user);
if (req.user?.sub) {
  const user = await this.usersService
    .findByExternalAuthId(req.user.sub)
    .catch((err) => {
      console.log('DEBUG - interceptor findByExternalAuthId FAILED:', err.message);
      return undefined;
    });
  userId = user?.id ?? null;
  console.log('DEBUG - interceptor resolved userId:', userId);
}
